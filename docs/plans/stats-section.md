...
- **Insertion Point**: Inside `<div className={styles.contentWrapper}>`, immediately **after** the `.heroCard` motion div and **before** the `.widgetContainer`.
- **Constraint**: Ensure it shares the same width constraints as the Hero for visual alignment.
