...
const GameDetails = () => {
    ...
    if (isLoading) return <div className={styles.loadingState}>{t("gameDetails.loading")}</div>;
    if (error || !game) return <div className={styles.errorState}>{t("gameDetails.gameNotFound")}</div>;
        ...
        <div className={styles.container}>
            ...
            <div className={styles.content}>
                <div className={styles.mainInfo}>
                    <Card>
                        <h2 className={styles.sectionTitle}>{t("gameDetails.aboutGame")}</h2>
                    ...
                    {game.assets?.screenshots && game.assets.screenshots.length > 0 && (
                        <Card>
                            <h2 className={styles.sectionTitle}>{t("gameDetails.screenshots")}</h2>
                ...
                <aside className={styles.sidebar}>
                    ...
                    <Card className={styles.priceCard} padding="lg">
                        <div className={styles.priceRow}>
                            ...
                            {hasOffer && <span className={styles.discountBadge}>{t("gameDetails.offer")}</span>}
                        </div>
                        ...
                        <div className={styles.actions}>
                            ...
                            <Button variant="ghost" disabled={!isAuthenticated} onClick={handleToggleWishlist}>
                                ...
                                {isWishlisted ? t("gameDetails.inWishlist") : t("gameDetails.addToWishlist")}
                            </Button>
                            {!isAuthenticated && (
                                <p className={styles.loginPrompt}>{t("gameDetails.loginToPurchase")}</p>
                            )}
                        </div>
                    </Card>
                    ...
                    <Card padding="md">
                        <div className={styles.detailRow}>
                            <span className={styles.detailLabel}>{t("gameDetails.genre")}</span>
                            <Link to={`/catalog?genre=${encodeURIComponent(game.genre)}`} className={styles.genreValue}>
                                {game.genre}
                            </Link>
                        </div>
                        <div className={styles.detailRow}>
                            <span className={styles.detailLabel}>{t("gameDetails.developer")}</span>
                            <Link
                                to={`/catalog?developer=${encodeURIComponent(game.developer)}`}
                        ...
                        <div className={styles.detailRow}>
                            <span className={styles.detailLabel}>{t("gameDetails.publisher")}</span>
                            <Link
                        ...
                        <div className={styles.detailRow}>
                            <span className={styles.detailLabel}>{t("gameDetails.releaseDate")}</span>
                            <span>
                                {game.releaseDate
                        ...
                        {game.metacritic && (
                            <div className={styles.detailRow}>
                                <span className={styles.detailLabel}>{t("gameDetails.metacritic")}</span>
                                <span className={styles.metacriticScore}>{game.metacritic}/100</span>
                            </div>
                        )}
                        {game.score && (
                            <div className={clsx(styles.detailRow, styles.userScore)}>
                                <span className={styles.detailLabel}>{t("gameDetails.userScore")}</span>
