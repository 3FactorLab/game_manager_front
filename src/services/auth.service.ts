...
const transformUser = (userData: any): User => {
    ...
    if (userData.profilePicture) {
        ...
        avatar =
            userData.profilePicture.startsWith("http") || userData.profilePicture.startsWith("/")
...
export const authService = {
    ...
    async getProfile(): Promise<User> {
        const { data } = await apiClient.get<{ message: string; user: User }>("/users/profile");
        const user = transformUser(data.user);
    ...
    async updateProfile(formData: FormData): Promise<User> {
        const { data } = await apiClient.put<{ message: string; user: User }>("/users/update", formData, {
            headers: {
    ...
    async refreshToken(): Promise<AuthResponse> {
        ...
        const { data } = await apiClient.post<AuthResponse>("/users/refresh-token", { token: refreshToken });
