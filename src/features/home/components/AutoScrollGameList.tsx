...
export const AutoScrollGameList: React.FC = () => {
    ...
    useEffect(() => {
        if (data?.pages[0]?.data) {
            // Flatten pages if needed, or just take the first batch
            const allGames = data.pages.flatMap((page) => page.data);
            // Duplicate functionality for seamless loop handled by CSS transform usually requires cloning
            // But here we'll just render enough items.
            // For a seamless vertical loop, we need to duplicate the content.
            setGames([...allGames, ...allGames]);
        }
    }, [data]);
