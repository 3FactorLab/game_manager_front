...
export const GameCard = ({ game, className, style }: GameCardProps) => {
     ...
     */
    const handleTagClick = (e: React.MouseEvent, key: "genre" | "developer" | "platform", value: string) => {
        e.stopPropagation();
    ...
    return (
        ...
        >
            ...
            <div className={styles.coverImageWrapper}>
                ...
                <button
                    className={`${styles.wishlistBtn} ${isInWishlist(game._id) ? styles.active : ""}`}
                    onClick={(e) => {
                        e.stopPropagation();
                    ...
                    title={isInWishlist(game._id) ? "Remove from wishlist" : "Add to wishlist"}
                >
            ...
            <div className={styles.content}>
                ...
                <div className={styles.header}>
                    ...
                    {game.score && <span className={styles.scoreBadge}>⭐ {game.score}</span>}
                </div>

                {/* Meta Info: Genre */}
                <div className={styles.metaContainer}>
                    <span className={styles.genreBadge} onClick={(e) => handleTagClick(e, "genre", game.genre)}>
                        {game.genre}
                    </span>
                </div>
                    ...
                    <div className={styles.priceContainer}>
                        ...
                        {(game.originalPrice || 0) > (game.price || 0) && (
                            <span className={styles.discount}>
                                -
                                {Math.round(
                                    (((game.originalPrice || 0) - game.price) / (game.originalPrice || 1)) * 100
                                )}
                                %
                            </span>
                        )}
                        ...
                        <span className={styles.price}>
                            {game.price === 0 ? t("catalog.free") : formatCurrency(game.price || 0, game.currency)}
                        </span>
                    </div>
                </div>
            </div>
