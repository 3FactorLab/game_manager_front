...
export const GameCard = ({ game }: GameCardProps) => {
    ...
    return (
        <Card className={styles.gameCard} padding="none" hoverable onClick={handleCardClick}>
            {/* Game cover image */}
            <div className={styles.coverImageWrapper}>
                ...
                >
                    {isInWishlist(game._id) ? <BsHeartFill color="var(--accent-primary)" /> : <BsHeart />}
                </button>
            </div>
            ...
            <div className={styles.content}>
                ...
                <div className={styles.badges}>
                    ...
                    {game.score && <span className={styles.scoreBadge}>ѓр? {game.score}/10</span>}
                </div>
                    ...
                    <div className={styles.priceContainer}>
                        ...
                        {hasOffer && (
                            <span className={styles.discount}>
                                -{Math.round(((game.price - (game.offerPrice || 0)) / game.price) * 100)}%
                            </span>
                        )}
                        {/* Original price (strikethrough) */}
                        {hasOffer && (
                            <span className={styles.oldPrice}>{formatCurrency(game.price, game.currency)}</span>
                        )}
                        {/* Current price */}
